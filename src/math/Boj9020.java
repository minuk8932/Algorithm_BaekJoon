...
public class Boj9020 {
	...
	public static void main(String[] args) throws Exception{
		...
		while(T-- > 0) {
			int n = Integer.parseInt(br.readLine());		// 골드바흐의 파티션을 찾아 낼 수
			...
			for(int i = n - 1; i >= n / 2; i--) {		// n에서 해당수의 n/2까지 돌려가면서
				...
				p = new Pair(diff, i);						// 소수가 맞다면 파티션 최대 최소 짝을 크기에 맞게 담은 후
			}
			
			sb.append(p.min).append(SPACE).append(p.max).append(NEW_LINE);	// 각각 버퍼에 저장
		}
