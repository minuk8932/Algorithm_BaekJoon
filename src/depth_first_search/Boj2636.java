...
public class Boj2636 {
	...
	public static void main(String[] args) throws Exception {
		...
		int timer = 0, res = 0, tmp = 0;
		
		while((tmp = getCheeseCount(N, M)) != 0) {			// 반복문을 돌면서 남은 치즈의 갯수를 확인	
			...
			timer++;						// 시간 계산
			res = tmp;						// 치즈가 모두 사라지기 직전까지 치즈 갯수 갱신
		}
		
		System.out.println(timer + "\n" + res);		// 치즈가 모두 녹은 시간과, 마지막 순간 남은 치즈의 갯수를 동시에 출력
	}
	...
	private static void removeCheese(int n, int m) {
		for(int i = 0; i < n; i++) {
			for(int j = 0; j < m; j++) {			// 녹이기 배열이 1이상인 경우, 외부와 최소 1군데 맞닿아 있음
